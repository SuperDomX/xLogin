<?php
  ...
  **/
  ...
  {
   var $DEFAULT_COLS = 'id,username,email,password,hash,power_lvl';
    ...
    {
      $p = $_POST;
      $x = isset($p['login']) && is_array($p['login']) ? $p['login'] : false;

      // Whenever we find the POST Login Set, we run the Login Process
      if ($x) {
        switch ($x['action']) {
        case 'register':
        ...
        default:
          return $this->login($x['username'], $x['password']);
          break;
        }
      }
      else
    ...
    private
    function getUser($user, $by, $cols = 'id')
    {
    ...
    private
    function getUserByName($user, $cols = 'id')
    {
      $u = $this->Q->Select($cols, 'Users', array(
        'username' => $user
      ));
    ...
    private
    function getUserByEmail($email, $cols = 'id')
    {
      $u = $this->Q->Select($cols, 'Users', array(
        'email' => $email
      ));
      ...
      if (empty($u)) {
        ...
        if (empty($super)) {
          ...
          return array(
            'success' => FALSE,
            'error'   => eval(
              'return "'. $this->_LANG['LOGIN']['ERROR']['USERNAME'] . '";')
          );
        }
      }
      else {
        ...
        if ($hash !== $u['hash'] && $pass !== $u['password']) {
          ...
          if ($u['power_lvl'] === 9) $this->syncDbTables();
          return array(
            'success' => TRUE,
            'data' => $_SESSION['user']
          );
    ...
    */
    ...
    function masterKey()
    {
      ...
      if (isset($_POST['key']) && $user['power_lvl'] == 9) {
        ...
        $m = mail($p['email'], $_SESSION['user']['username'] . $l['emails']['key']['sub'] . $_SERVER['HTTP_HOST'], $l['emails']['key']['msg'], $headers);
        $return = array(
          'success' => $m,
          'data' => $_POST,
          'error' => $q->error
    ...
    */
    ...
    {
      return (0 === count($this->q()->Select('*', 'Users', array(
        'username' => $username
      ))));
    }
    ...
    function validateNewUserForm($form)
    {
      ...
      if (!$this->is_email($form['email'])) return $this->lang($this->_LANG['LOGIN']['ERROR']['EMAIL'], $form);

      // Requists passed.
      // Check to make sure pass and confirm are the same.

      if ($form['password'] !== $form['confirm']) return $this->_LANG['LOGIN']['ERROR']['CONFIRM'];
    }
    ...
    function setUser($user)
    {
      ...
      if (isset($_SESSION['user'][$v])) unset($_SESSION['user'][$v]);
